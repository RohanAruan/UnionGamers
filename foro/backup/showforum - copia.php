...
if ($user_id) {
mysqli_query($con, "UPDATE onlines SET tiempo='$this_date', idforo='$_GET[s]', idsforo='0', idtema='0' WHERE eid = '$iduser'");
} else {
mysqli_query($con, "INSERT onlines SET eid = '$iduser', idforo='$_GET[s]', tiempo = '$this_date'");
}

mysqli_query($con, "UPDATE aor_logs SET idforo = '0' WHERE idforo = '$_GET[s]' AND idusuario = '$iduser'");
?>
...
<?php 
$sqlA = mysqli_query($con, "SELECT * FROM aor_categorias WHERE id = '$idcategoria'");
...
<a href="createtopic?c=<?=$notc['id']?>" class="btn btn-blue btn-sm"><i style="font-size:11px" class="fa fa-plus"></i> Crear nuevo tema</a>
</div>
</div>
</div>
...
<?php
$subforosx = $con->query("SELECT * FROM aor_subforos WHERE idforo = '$_GET[s]' AND idsub = '0'");
...
<div class="content">
<a href="showsforum?s=<?=$subforos['id']?>-<?=$subforos['nombre']?>" style="text-transform:uppercase;font-size:14px;" class="h6 title"><?=$subforos['nombre']?></a>
...
<a href="#" style="color:#38a9ff" class="h6 title"><?=$temas['nombre']?></a> 
...
<?php
$temasx = $con->query("SELECT * FROM aor_temas WHERE idlider = '$_GET[s]' ORDER BY fijado DESC LIMIT 25");
...
<tr style="border: 1px solid #CECECE;background:url(<?php if($fijado != 0) {?>https://foro.unplayer.com/images/gradients/bg-sticky.png<?php } ?>)" class="bg-smoke-light <?php if($fijado != 0) {?>bg-fijado<?php } ?>">
<td class="forum">
<div class="forum-item">
...
<?php
$usuarios = mysqli_query($con, "SELECT * FROM onlines WHERE eid AND idforo = '$_GET[s]'");
...
<img style="width:25px;height:25px;border-radius:50%;object-fit:cover" src="../estilos/img/skins/<?=$user['Skin'];?>.jpg" data-toggle="tooltip" data-placement="top" title="<?php $quitargion=str_replace("_"," ",$user['Nombre_Apellido']); echo(''.$quitargion.'') ?>"> 
</a>
